...
public class OpenWeatherMapDataSourceTest {
    ...
    public void getWeatherAsync_parseCorrectlyJsonObjectResponse() {
        ...
        CoordinateEntity mockCoordinate = MockerHelper.mockCoordinateEntity();
        LiveData<WeatherRemoteDto> actual = mockDataSource.getWeatherAsync(mockCoordinate);
        ...
        Assert.assertEquals(expectedTemperature, actual.getValue().getTemperatureInCelsius(), ConstantsHelper.DELTA);
        Assert.assertEquals(expectedSkyCondition, actual.getValue().getSkyCondition());
        Assert.assertEquals(expectedTuesdayMaxTemp, actual.getValue().getForecast().get(0).getMaximumTemperatureInCelsius(), ConstantsHelper.DELTA);
        Assert.assertEquals(expectedTuesdayMinTemp, actual.getValue().getForecast().get(0).getMinimumTemperatureInCelsius(), ConstantsHelper.DELTA);
    }
